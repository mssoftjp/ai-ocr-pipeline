...
`-d` や `--run-root` で出力すると、検出結果を元画像に重ねた確認用画像が自動生成されます。信頼度に応じて色分けされ、読み取り結果を目視で確認できます。
...
| `--ocr-split-gap-sensitivity` | 1.0 | 空白 gap 分割の感度。大きいほど細かく分割。`direct` 専用 |

`--ocr-split-gap-sensitivity` は `0` 以下を受け付けません。`0.1` 未満は `0.1` にクランプされます。`--ocr-backend subprocess` では無視されます。
...
表のセルが一つにまとまって検出されるときは、分割感度を上げると改善することがあります。

```bash
ai-ocr-pipeline invoice.pdf \
  --ndl \
  --ocr-split-gap-sensitivity 2.0 \
  -d results/
```
