# ai-ocr-pipeline — AI アシスト 日本語OCR

画像や PDF から日本語テキストを高精度に読み取り、座標付き JSON で出力するツールです。

国立国会図書館の [ndlocr-lite](https://github.com/ndl-lab/ndlocr-lite) をベースに、**LM Studioによるローカル LLM の AI 補正**を組み合わせることで、OCR 単体では読み間違いやすいスキャン帳票・手書き混在文書の精度を大幅に向上させます。

## 特徴

- **AI アシスト OCR** — OCR の検出座標を保ったまま、LLM（LM Studio 経由）がテキストを補正。LLM処理まで必要でない部分などは ndlocr-lite のみで動作するため、AI 環境なしでも使える
...
- **構造化 JSON 出力** — 出力結果に座標データを持たせることで、画像なしでテキストだけで軽量にレイアウトを把握でき、文書構造の理解力が向上する。RPA やデータ連携にもそのまま組み込みやすい
- **各種ファイルの入力対応** — PDF/PNG/JPEG/TIFFに対応
- **画像にオーバーレイで確認** — 検出結果を元画像に重ねた画像を自動出力し、目視確認が容易
...
# PDF を読み取り、結果を pretty-print
ai-ocr-pipeline document.pdf --pretty
...
LM Studio が起動していれば **自動的に AI 補正が有効** になります。起動していなければ OCR のみで動作します。
...
| オプション | 推奨値 | 理由 |
|-----------|--------|------|
| `--llm-max-tokens` | 256 | 帳票テキストは短い。大きすぎるとノイズ画像で暴走する |
| `--llm-max-workers` | 16 | LM Studio のスロット数に合わせる。多すぎると逆に遅くなる |
...
| Max Tokens | 256〜512 | サーバー側でも上限を絞ると安全 |
...
| `-d results/` | JSON + overlay 画像をディレクトリ保存 |
...
| `--prefer-text-layer` | PDF 埋め込みテキストを優先使用 |
...
テンプレートモードは AI による読み取りが前提のため、`--ndl`（OCR のみ）とは併用できません。
...
# 2. エディタで result.json を「OCR結果を取り込む」から読み込み
...
`-d` や `--run-root` で出力すると、検出結果を元画像に重ねた overlay 画像が自動生成されます。信頼度に応じて色分けされ、読み取り結果を目視確認できます。
...
| `--overlay` | overlay 画像を生成（`-o` 時） |
| `--include-absolute-geometry` | pixel 座標を JSON に追加 |
| `--include-debug-fields` | 診断フィールドを JSON に追加 |
...
|-----------|-----------|------|
| `--deskew` | off | 傾き補正 |
| `--remove-horizontal-lines` | off | 横罫線除去 |
| `--remove-vertical-lines` | off | 縦罫線除去 |
| `--dpi` | 600 | PDF ラスタライズ DPI |
| `--prefer-text-layer` | off | PDF 埋め込みテキスト優先 |
...
| `--ocr-filter-container-fallbacks` | on | 誤検出の巨大要素を除去 |
| `--ocr-split-wide-lines` | on | テーブル行をセル単位に分割 |
...
```bash
ai-ocr-pipeline image.png --ndl --pretty
```

### PDF の埋め込みテキストを使う

```bash
ai-ocr-pipeline document.pdf --prefer-text-layer --pretty
```
...
ただし、OCR 実行時には `ndlocr-lite` に依存します。`ndlocr-lite` は National Diet
Library, Japan により `CC BY 4.0` で公開されています。

- Python パッケージ版:
  - `ndlocr-lite` は依存関係として別途インストールされます
